...
# Count each group by charge range
cat("Number of people with charges < 10000:", nrow(df %>% filter(charges < 10000)), "\n")
cat("Number of people with charges between 10000 and 30000:", nrow(df %>% filter(charges >= 10000 & charges <= 30000)), "\n")
cat("Number of people with charges > 30000:", nrow(df %>% filter(charges > 30000)), "\n")
...
# Train/test split
index <- createDataPartition(df$charges_group, p = 0.7, list = FALSE)
train <- df[index, ]
test <- df[-index, ]

# Classification models
models <- list(
  DecisionTree = rpart::rpart(charges_group ~ ., data = train),
  KNeighbors = kknn::train.kknn(charges_group ~ ., data = train, k = 5), 
  NaiveBayes = e1071::naiveBayes(charges_group ~ ., data = train),
  RandomForest = randomForest(charges_group ~ ., data = train)
)

# Model Evaluation
results <- sapply(models, function(model) {
  # Prédiction
  pred <- if (inherits(model, "train.kknn")) {
    predict(model, test)  # kknn retourne directement les prédictions sans 'type = "class"'
  } else {
    predict(model, test, type = "class")  # Pour les autres modèles, spécifiez le type de prédiction
  }
  
  # Calcul de l'exactitude
  accuracy <- mean(pred == test$charges_group)
  return(accuracy)
})
